...
source2 = cv2.cvtColor(source, cv2.COLOR_BGR2LAB).astype("float32")
(l,a,b) = cv2.split(source2)
...
show_image("Source", sourcer)
show_image("Target", sourceg)
show_image("Transfer", sourceb)
cv2.imwrite("sourcer.jpg", sourcer)
cv2.imwrite("sourceg.jpg", sourceg)
cv2.imwrite("sourceb.jpg", sourceb)
cv2.waitKey(0)
